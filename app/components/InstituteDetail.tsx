...
// function TabPhd({ sections, allSections }: { sections: RawSection[]; allSections: RawSection[] }) {
//   if (!sections.length) return <Empty msg="PhD data not available for this ranking year." />;
...
// function TabStudents({ sections, allSections }: { sections: RawSection[]; allSections: RawSection[] }) {
//   if (!sections.length) return <Empty />;
...
// function TabResearch({ sections, allSections, row }: { sections: RawSection[]; allSections: RawSection[]; row: Record<string, unknown> | null }) {
//   if (!sections.length) return <Empty />;
//   const allSecMap = new Map(allSections.map(s => [s.section, s]));
//   return (
//     <div>
...
//         }}>
//           {row.pdf_sponsored_projects != null && (
//             <KV label="Sponsored Projects (3yr)" value={Number(row.pdf_sponsored_projects).toLocaleString("en-IN")} accent />
//           )}
//           {row.pdf_consultancy_projects != null && (
//             <KV label="Consultancy Projects (3yr)" value={Number(row.pdf_consultancy_projects).toLocaleString("en-IN")} />
//           )}
//           {row.pdf_edp_participants != null && (
//             <KV label="EDP Participants (3yr)" value={Number(row.pdf_edp_participants).toLocaleString("en-IN")} />
//           )}
//         </div>
//       )}
//       {sections.map(sec => (
...
//         if (hasNamedProgram) {
//           return <Card key={sec.section} title={sec.section} noPad><RecordTable metrics={metrics} nameCol="Name / Year" /></Card>;
//         }
...
// function TabFinancial({ sections, allSections, row }: { sections: RawSection[]; allSections: RawSection[]; row: Record<string, unknown> | null }) {
//   if (!sections.length) return <Empty />;
//   const allSecMap = new Map(allSections.map(s => [s.section, s]));
//   return (
//     <div>
...
//           {row.pdf_capital_expenditure != null && (
//             <KV label="Capital Expenditure (3yr Sum)" value={fmtAmt(String(row.pdf_capital_expenditure))} accent />
//           )}
//           {row.pdf_operational_expenditure != null && (
//             <KV label="Operational Expenditure (3yr Sum)" value={fmtAmt(String(row.pdf_operational_expenditure))} />
//           )}
//         </div>
//       )}
//       {sections.map(sec => (
...
//                     {hasYear && m.year && m.year !== "-" && (
//                       <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.62rem", color: "var(--ink-300)", marginLeft: 8 }}>({m.year})</span>
//                     )}
//                   </div>
//                   <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.7rem", flexShrink: 0, padding: "3px 10px", color: yes ? "var(--teal)" : "var(--crimson)", background: yes ? "var(--teal-pale)" : "var(--crimson-pale)", border: `1px solid ${yes ? "rgba(26,122,110,0.2)" : "rgba(192,57,43,0.2)"}` }}>
//                     {m.value}
...
//         if (!metrics.length) return null;
//         const hasProgram = metrics.some(m => !isBAD(m.program) && cl(m.program) !== "-");
//         if (hasProgram) return <Card key={sec.section} title={sec.section} noPad><RecordTable metrics={metrics} nameCol="Body" /></Card>;
//         return <Card key={sec.section} title={sec.section} noPad><FlatTable metrics={metrics} /></Card>;
//       })}
//     </div>
//   );
...
//           if (rows.length) return <Card key={sec.section} title={sec.section} noPad><YearPivotTable rows={rows} years={years} /></Card>;
//         }
//         if (hasProgram) return <Card key={sec.section} title={sec.section} noPad><RecordTable metrics={metrics} /></Card>;
//         return <Card key={sec.section} title={sec.section} noPad><FlatTable metrics={metrics} /></Card>;
//       })}
//     </div>
//   );
...
//         } else {
//           // Pick the category of the most recent year's row
//           const firstRow = d.scoresByYear[yrs[0]] as Record<string, unknown> | undefined;
//           setActiveCategory((firstRow?.category as string) ?? d.categories[0] ?? "");
//         }
...
//   const allCategoriesForInst = [...new Set(
//     (Object.values(profile.scoresByYear) as Record<string, unknown>[])
//       .map(r => r.category as string)
//       .filter(Boolean)
//   )].sort((a, b) => {
//     const ORDER: Record<string, number> = {
//       "Overall": 0, "University": 1, "Engineering": 2, "Management": 3,
//       "Research": 4, "Medical": 5, "College": 6, "Pharmacy": 7,
//       "Law": 8, "Architecture": 9,
//     };
...
//     ? (() => {
//         const allRows = Object.values(profile.scoresByYear) as Record<string, unknown>[];
//         if (activeCategory) {
//           const catRow = allRows.find(r =>
//             Number(r.ranking_year) === activeYear && r.category === activeCategory
//           );
//           if (catRow) return catRow;
//         }
//         return (profile.scoresByYear[activeYear] as Record<string, unknown>) ?? null;
//       })()
//     : null;

//   // Filter raw sections by active category
//   const allRawSections = profile.rawSections as RawSection[];
//   const rawSections: RawSection[] = allRawSections.map(sec => ({
//     ...sec,
//     metrics: activeCategory
//       ? sec.metrics.filter(m => !m.category || m.category === activeCategory)
//       : sec.metrics,
...
//                 fontStyle: "italic",
//                 fontSize: "3.2rem",
//                 color: activeCategory ? catColor(activeCategory) : "var(--crimson)",
//                 lineHeight: 1,
//                 marginBottom: 3,
//               }}>
//                 {(row.img_total as number).toFixed(2)}
...
//                 : "2px solid transparent",
//               marginBottom: "-2px",
//               color: safeTab === tab.id
//                 ? (activeCategory ? catColor(activeCategory) : "var(--crimson)")
//                 : "var(--ink-400)",
//               cursor: "pointer",
//               whiteSpace: "nowrap",
//               transition: "all 0.15s",
...
//       {/* ── Tab body ── */}
//       <div key={`${safeTab}-${activeYear}-${activeCategory}`} style={{ animation: "fadeIn 0.18s ease both" }}>
//         {tabContent[safeTab]}
//       </div>
//     </div>
//   );
