...
//                 const paramKey = value.slice(sep + 2);
//                 const pLabel =
//                   ALL_SCORE_PARAMS.find((p) => p.key === paramKey)?.short ??
//                   paramKey;
//                 const instName = profiles[code]?.institute_name ?? code;
//                 const cat = instCategories[code];
//                 return `${instName}${cat ? ` (${cat})` : ""} · ${pLabel}`;
...
//           <PolarAngleAxis
//             dataKey="param"
//             tick={{ fontFamily: MONO, fontSize: 9, fill: INK500 }}
//           />
//           {codes.map((code, i) => (
//             <Radar
//               key={code}
...
//               return <span style={{ color: INST_COLORS[idx] }}>{name}</span>;
//             }}
//           />
//         </RadarChart>
//       </ResponsiveContainer>
...
//       setInstCategories((prev) => ({ ...prev, [code]: cat }));
//       // Reset year to most recent available for this category
...
//         .filter((n) => !isNaN(n))
//         .sort((a, b) => b - a);
//       setInstYears((prev) => ({ ...prev, [code]: availYears[0] ?? 0 }));
//     },
//     [data],
//   );
...
//                       if (availYears.length <= 1) return null;
//                       return (
...
//         instCategories={instCategories}
//       />
//       {instYears[loadedCodes[0]] && (
//         <ScoreRadar
//           profiles={data}
//           codes={loadedCodes}
//           year={instYears[loadedCodes[0]]}
//           instCategories={instCategories}
//         />
//       )}
...
//     const catKey = `${year}::${category}`;
//     if (sby[catKey]) return sby[catKey];
//     // Don't fall back to plain year key if category is specified
//     // — that would show wrong category's data
//     return null;
//   }
//   return sby[String(year)] ?? null;
// }
