...
// interface ExamRawMetric extends RawMetric { ranking_year: number; }

// const EXAM_SERIES = [
//   { match: ["admitted in the first year", "admitted in first year"], label: "Admitted (1st Year)", color: PALETTE[0] },
//   { match: ["lateral entry", "admitted through lateral"],            label: "Lateral Entry",       color: PALETTE[1] },
//   { match: ["graduating in minimum", "graduating in min"],           label: "Graduating (Min Time)", color: PALETTE[2] },
// ];
...
// export function UniversityExamTrendChart({ metrics }: { metrics: ExamRawMetric[] }) {
//   // Collect all distinct programs
//   const progSet = new Set<string>();
//   for (const m of metrics) {
//     if (!isBAD(m.program) && m.program.trim() !== "-") progSet.add(m.program.trim());
...
//     const yr = isRealYear(m.year) ? baseYear(m.year)
...
//     validProg === ALL_PROG
//       ? allMetrics
//       : allMetrics.filter((m) => m.program.trim() === validProg);
//   const yearSet = new Set<string>();
//   const mmap = new Map<string, Map<string, number>>();
...
//     validProg === ALL_PROG
//       ? metrics
//       : metrics.filter((m) => m.program.trim() === validProg);
