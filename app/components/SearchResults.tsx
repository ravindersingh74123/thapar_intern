...
/**
 * SearchResults.tsx
 * Shows one row per institute name — clicking opens the detail page directly.
 * In compare mode, clicking adds the institute to the comparison tray.
 */
import type { SearchHit } from "@/app/page";

// The grouped shape returned by /api/institute-search
export interface SearchGroup {
  institute_name: string;
  best_score:     number | null;
  entries: {
...
export default function SearchResults({ groups, onSelect, compareMode, selectedCodes = [] }: Props) {
  return (
    <div>
      {groups.map((group, gi) => {
        // Pick the best entry (highest score, Overall preferred)
        const best = group.entries.sort((a, b) => {
          const catScore = (c: string) => c === "Overall" ? 0 : c === "University" ? 1 : c === "Engineering" ? 2 : 99;
          if (catScore(a.category) !== catScore(b.category)) return catScore(a.category) - catScore(b.category);
          return (b.img_total ?? 0) - (a.img_total ?? 0);
        })[0];

        const hit: SearchHit = {
          institute_code: best.institute_code,
          institute_name: group.institute_name,
          category:       best.category,
          best_year:      best.best_year,
          img_total:      best.img_total,
        };
        const isAdded = selectedCodes.includes(best.institute_code);
        ...
        return (
          <button
            ...
            style={{
              ...
              borderBottom: gi < groups.length - 1 ? "1px solid var(--border)" : "none",
              cursor: isAdded ? "default" : "pointer", textAlign: "left",
              transition: "background 0.1s",
              animation: `fadeUp 0.2s var(--ease-out) ${gi * 30}ms both`,
            }}
            onMouseEnter={e => { if (!isAdded) e.currentTarget.style.background = "var(--off-white)"; }}
            onMouseLeave={e => { e.currentTarget.style.background = "transparent"; }}
          >
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ fontFamily: "var(--font-body)", fontWeight: 600, fontSize: "0.88rem", color: "var(--ink-900)", marginBottom: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                {group.institute_name}
              </p>
            </div>
            {group.best_score != null && (
              <div style={{ textAlign: "right", flexShrink: 0 }}>
                <p style={{ fontFamily: "var(--font-mono)", fontSize: "1rem", fontWeight: 500, color: "var(--ink-900)", lineHeight: 1 }}>
                  {group.best_score.toFixed(1)}
                </p>
                <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.6rem", color: "var(--ink-300)", marginTop: 2 }}>NIRF Score</p>
              </div>
            )}
            {compareMode
              ? <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.65rem", color: isAdded ? "var(--ink-300)" : "var(--crimson)", border: `1px solid ${isAdded ? "var(--border)" : "var(--crimson)"}`, padding: "3px 8px" }}>{isAdded ? "Added" : "+ Add"}</span>
              : <span style={{ color: "var(--ink-100)", fontSize: "0.9rem" }}>→</span>
            }
          </button>
        );
      })}
