...
// /**
//  * app/api/lib/db.ts
//  * Uses better-sqlite3 — pure JS, no native glibc dependencies, works on Vercel.
//  */

// import path from "path";
// import fs   from "fs";

// const DB_PATH    = path.join(process.cwd(), "public/data/nirf.db");
// const META_PATH  = path.join(process.cwd(), "public/data/nirf_meta.json");
...
// function getDb() {
//   if (_db) return _db;
//   // Dynamic import keeps native module out of build-time analysis
//   // eslint-disable-next-line @typescript-eslint/no-var-requires
//   const Database = require("better-sqlite3");
//   if (!fs.existsSync(DB_PATH)) {
//     throw new Error(`SQLite DB not found at ${DB_PATH}. Run: python scripts/build-db.py`);
//   }
...
//   const db   = getDb();
//   const stmt = db.prepare(sql);
//   const row  = params.length === 0 ? stmt.get() : stmt.get(...params);
//   return (sanitise(row) as T) ?? null;
// }
...
// export function dbFilesExist() {
//   return {
//     raw:        fs.existsSync(path.join(process.cwd(), "public/data/nirf_raw.parquet")),
//     scores:     fs.existsSync(path.join(process.cwd(), "public/data/nirf_scores.parquet")),
//     meta:       fs.existsSync(META_PATH),
//     db:         fs.existsSync(DB_PATH),
//     rawPath:    path.join(process.cwd(), "public/data/nirf_raw.parquet"),
//     scoresPath: path.join(process.cwd(), "public/data/nirf_scores.parquet"),
//   };
// }